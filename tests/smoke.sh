...
grep -Fq 'name="brandOverride"' "${temporary_dir}/admin-edit.html"
...
status=$(curl --silent --output "${temporary_dir}/edit-response.html" --write-out '%{http_code}' \
    ...
    --data-urlencode 'modelOverride=Creality Ender 3' --data-urlencode 'brandOverride=Creality' \
...
$php_bin -r '
    ...
    if (($printers[1]["modelOverride"] ?? "") !== "Creality Ender 3"
        || ($printers[1]["brandOverride"] ?? "") !== "Creality") {
        fwrite(STDERR, "Model or brand override was not persisted.\n");
        exit(1);
    }
' "$printers_file"
